function calculateAIC_HSM_OneMinusTAC_conseGap(varargin)
    ...
    default_display_ids = [1:45];  % plot ids to analyze
    % default_display_ids = [1:47,50,51,53];  % plot ids to analyze, remove HSF (48 and 49) due to sparse observation, remove PNM (52) due to the lack of climate data
    % default_display_ids = [1:46,48,50,51,53];          % remove ENSO years LSF and HSF, remove PNM (52) due to the lack of climate data

    default_save_fig = 0;               % do not save figures
    default_save_output = 1;            % do not save R2 output
    default_use_p88 = 0;                % use HSM p88 instead of HSM p50
    default_do_plot = 0;                 % display plots
...
% 
    ...
    HSM_p50_slopes = zeros([length(VIs),length(rolling_windows_y)]);   
    HSM_p50_pValue = zeros([length(VIs),length(rolling_windows_y)]);  

    HSM_p88_r_results = zeros([length(VIs),length(rolling_windows_y)]);  
    HSM_p88_pValue = zeros([length(VIs),length(rolling_windows_y)]);  
    ...
    for ic = 1:length(composite_intervals)
        ...
        for iV = 1:length(VIs)
            ...
            for ir = 1:length(rolling_windows_y)
                ...
                mean_def = groupsummary(def, sampleID, "mean")*365*1000;  % annual mean of def
                ...
                for i = 1:length(point_ids)
                    ...
                    if ~isempty(nearestIndex)
                        ...
                        [~, nearestIndex] = min(abs(TAC_annual.Dates - datetime(year_field,month_field,1)));
                    ...
                    % % calculate valid data pct within the rolling window
                    % tmp = TAC_record_change.(['TAC_',composite_interval]).('data_count');
                    % switch composite_interval
                    %     case 'biweekly'
                    %         scale = 26;
                    %     case 'monthly'
                    ...
                    %         scale = 4;
                    % end
                    % tmp = tmp(nearestIndex-rolling_window_y*scale:nearestIndex);
                    % valid_data_pct = length(find(tmp>0))./length(tmp);
                    % fprintf('plot name : %s\n',plotname)
                    % fprintf('Valid data percentage is %.3f%%\n',valid_data_pct*100);
                ...
                for j = 1:length(unique_names)
                    ...
                    % Calculate the mean
                    % ks 20250831 use nanmean instead
                    ...
                    % Calculate water deficit for the current group  
                    group_data_3 = DEF(strcmp(plot_names, unique_names{j}));
                    mean_DEF(j) = mean(group_data_3);
                end

                %% Fit OLS regression HSM (x) and 1-TAC (y): y = b0+b1*x
                [R,P] = corrcoef(mean_HSM(:),1-mean_TAC(:), 'Rows', 'complete');  % ignores NaNs
                r = R(1,2);
                p_value = P(1,2); 
              
                mdl_1 = fitlm(mean_HSM, 1-mean_TAC); 
                aic_1 = mdl_1.ModelCriterion.AIC;
                bic_1 = mdl_1.ModelCriterion.BIC;

                %% Fit additive fixed-intercept by CWD model: y = b0+b1*x+b2*CWD
                mdl_2 = slope_common_intercept_water(mean_HSM',1-mean_TAC',mean_DEF');
                aic_2 = mdl_2.ModelCriterion.AIC;
                bic_2 = mdl_2.ModelCriterion.BIC;
                                
                if msg
                    if p_value<0.01
                        % fprintf('HSM vs TAC slope = %.3f** \n',slope);
                        fprintf('HSM vs TAC r = %.3f** \n',r);
                    elseif p_value<0.05
                        % fprintf('HSM vs TAC slope = %.3f* \n',slope);
                        fprintf('HSM vs TAC r = %.3f* \n',r);
                    else
                        % fprintf('HSM vs TAC slope = %.3f, not significant at the 5%% significance \n',slope);
                        fprintf('HSM vs TAC r = %.3f, not significant at the 5%% significance \n',r);
                    end
                    fprintf('Model 1 AIC: %.3f\n',aic_1);
                    fprintf('Model 2 AIC: %.3f\n',aic_2);
                    if aic_1 < aic_2
                        disp('Model 1 is preferred (lower AIC).');
                    else
                        disp('Model 2 is preferred (lower AIC).');
                    end
                end
                ...
                if use_p88
                    HSM_p88_r(iV,ir) = r;
                    HSM_p88_pValues(iV,ir) = p_value;
                else

                    HSM_p50_r(iV,ir) = r;
                    HSM_p50_pValues(iV,ir) = p_value;

                    HSM_p50_AIC1(iV,ir) = aic_1;
                    HSM_p50_AIC2(iV,ir) = aic_2;

                    HSM_p50_R2_mdl1(iV,ir) = mdl_1.Rsquared.adjusted;
                    HSM_p50_R2_mdl2(iV,ir) = mdl_2.Rsquared.adjusted;

                    HSM_p50_N(iV,ir) = sum(~isnan(mean_TAC));
    ...
    folderpath_output = fullfile(directory,'r_TAC_HSM_AIC_conseGapTest',composite_interval);
    if ~exist(folderpath_output)
        mkdir(folderpath_output);
    end
    if save_output
        if use_p88
            save(fullfile(folderpath_output,'HSM_p88_r.mat'),"HSM_p88_r");
            save(fullfile(folderpath_output,'HSM_p88_pValues.mat'),"HSM_p88_pValues");
        else
            
            save(fullfile(folderpath_output,['HSM_p50_r_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_r");
            save(fullfile(folderpath_output,['HSM_p50_pValue_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_pValues");

            save(fullfile(folderpath_output,['HSM_p50_AIC1_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_AIC1");
            save(fullfile(folderpath_output,['HSM_p50_AIC2_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_AIC2");

            save(fullfile(folderpath_output,['HSM_p50_R2_mdl1_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_R2_mdl1");
            save(fullfile(folderpath_output,['HSM_p50_R2_mdl2_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_R2_mdl2");
            
            save(fullfile(folderpath_output,['HSM_p50_N_','conseGap',int2str(conseGap),'.mat']),"HSM_p50_N");
    ...
    if do_plot
        ...
        for ip = 1:length(unique_names)
            scatter(mean_HSM(ip),1-mean_TAC(ip),sz,'filled');
            text(mean_HSM(ip)-0.1,1-mean_TAC(ip)+0.01,unique_names{ip});
            hold on;
        end
        
        % Plot simple linear regression
        mdl_1 = fitlm(mean_HSM,1-mean_TAC);  
        x_fit = linspace(min(mean_HSM), max(mean_HSM), 100); % Create 100 evenly spaced x values
        [y_fit,y_ci] = predict(mdl_1, x_fit'); % Predict y values using the model
        ...
        % add text
        dim = [.2 .01 .3 .3];
        str = sprintf('R^2 = %.2f',mdl_1.Rsquared.Ordinary);
