function calculateLinearRegressionSlope_HSM_OneMinusTAC(varargin)
...
% 
    ...
    for ic = 1:length(composite_intervals)
        ...
        for iV = 1:length(VIs)
            ...
            for ir = 1:length(rolling_windows_y)
                ...
                mdl_3 = fitlm(mean_HSM, 1-mean_TAC); 
                slope = mdl_3.Coefficients.Estimate(2);
                % p_value = mdl_3.Coefficients.pValue(2);
                if msg
                    if p_value<0.01
                ...
                if use_p88
                    HSM_p88_slopes(iV,ir) = slope;
                    HSM_p88_r(iV,ir) = r;
                    HSM_p88_pValues(iV,ir) = p_value;
                else
                    HSM_p50_slopes(iV,ir) = slope;
                    HSM_p50_r(iV,ir) = r;
    ...
    if save_output
        ...
        if use_p88
            save(fullfile(folderpath_output,'HSM_p88_slopes.mat'),"HSM_p88_slopes");
            ...
            save(fullfile(folderpath_output,'HSM_p50_slopes.mat'),"HSM_p50_slopes");
