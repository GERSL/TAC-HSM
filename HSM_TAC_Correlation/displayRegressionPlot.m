function displayRegressionPlot(varargin)
        ...
        for ic = 1:length(composite_intervals)
            ...
            for ir = 1:length(rolling_windows_y)
                ...
                if remove_climateTAC
                    TAC = TAC-climate_ac_effect;%diff(display_ids);
                end
                %% Use absolute values
                if use_abs
                    TAC = abs(TAC);
    ...
    folderpath_output = fullfile(directory,['R2_TAC_HSM_2025-08-01'],composite_interval);
    ...
    if do_plot
        ...
        for ip = 1:length(unique_names)
            ...
            text(mean_TAC(ip)-0.02,mean_HSM(ip)-0.1,unique_names{ip});
            hold on;
        end
