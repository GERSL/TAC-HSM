function TAC_record_change = autoTAC_sample(sensor,plot_data, varargin)
%%---------------------------------------------------------------------------------------
    ...
    if ~any(line_t(:,:),'all')  
        ...
        for iC = 1:length(Composite_Intervals)
            ...
            if do_plot  % plot time series of vegetation index
                ...
                if pt_lat>0
                    if pt_lon>0
                        title(sprintf('PointId=%03d, %s (%.3f°N, %.3f°E)',pt_id,pt_name,pt_lat,pt_lon));
                    else
                        title(sprintf('PointId=%03d, %s (%.3f°N, %.3f°W)',pt_id,pt_name,pt_lat,abs(pt_lon)));
                    end
                else
                    if pt_lon>0
                        title(sprintf('PointId=%03d, %s (%.3f°S, %.3f°E)',pt_id,pt_name,abs(pt_lat),pt_lon));
                    else
                        title(sprintf('PointId=%03d, %s (%.3f°S, %.3f°W)',pt_id,pt_name,abs(pt_lat),abs(pt_lon)));
                    end
                end      
                legend([h1,h2],'Location','southeastoutside');
                ax = gca;
                ax.XLim  = datenum([datetime(2000,1,1) datetime(2024,12,31)]);
                ax.XTick = datenum(datetime(2000,1,1):calyears(2):datetime(2024,12,31)); % every 2 years
                datetick(ax,'x','yyyy','keepticks','keeplimits');
                xtickangle(ax,0);
            ...
            for iV = 1:length(Vegetation_Indices)
                ...
                for iw = 1:length(rolling_window)
                    ...
                    switch VI
                        case 'NDVI'
                            tmp = calEmprAC(TT3.NDVI, TT3.data_count, rolling_window(iw));
                        case 'kNDVI'
                            tmp = calEmprAC(TT3.kNDVI, TT3.data_count, rolling_window(iw)); 
                        case 'NIRv'
                            tmp = calEmprAC(TT3.NIRv, TT3.data_count, rolling_window(iw)); 
                        case 'NBR'
                            tmp = calEmprAC(TT3.NBR, TT3.data_count, rolling_window(iw)); 
                        case 'NDMI'
                            tmp = calEmprAC(TT3.NDMI, TT3.data_count, rolling_window(iw)); 
                        case 'EVI'
                            tmp = calEmprAC(TT3.EVI, TT3.data_count, rolling_window(iw)); 
                        case 'EVI2'
                            tmp = calEmprAC(TT3.EVI2, TT3.data_count, rolling_window(iw)); 
                    ...
                    if do_plot && iw>=4
                        if startsWith(var_name,['TAC_',plot_VI,'_'])
                            ...
                            plot(TT3.Dates,TT3.(var_name),'LineWidth',1.5,'DisplayName',[num2str(rolling_window(iw)/factor),' year']);
                            hold on
                            ...
                            if iw==length(rolling_window)
                                ...
                                % end
                                ax = gca;
                                ax.XLim  = [datetime(2000,1,1) datetime(2024,12,31)];
                                ax.XTick = datetime(2000,1,1):calyears(2):datetime(2024,12,31); % every 2 years
                                datetick(ax,'x','yyyy','keepticks','keeplimits');
                                xtickangle(ax,0);
            ...
            switch composite_interval
                case 'weekly'
                    TAC_record_change(N).TAC_weekly = TT3;
                case 'biweekly'
                    TAC_record_change(N).TAC_biweekly = TT3;
                case 'monthly'
                    TAC_record_change(N).TAC_monthly = TT3;
                case '48days'
                    TAC_record_change(N).TAC_48days = TT3;
                case '50days'
                    TAC_record_change(N).TAC_50days = TT3;
                case 'bimonthly'
                    TAC_record_change(N).TAC_bimonthly = TT3;
                case '70days'
                    TAC_record_change(N).TAC_70days = TT3;
                case 'quarterly'
                    TAC_record_change(N).TAC_quarterly = TT3;
            end
