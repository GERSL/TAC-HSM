function obtainEnhancedTAC()
    ...
    rolling_windows_y = 7;%[1,2,3,4,5,6,7];
    ...
    for iV = 1:length(VIs)
        ...
        for ic = 1:length(composite_intervals)
            ...
            for ir = 1:length(rolling_windows_y)
                ...
                %% Load original TAC
                filename = fullfile(directory,'Input',response_var_Inyear,'Sample_multipleInPlot_HPC_input_updated.csv');
                T = readtable(filename);
                TAC = T.(response_var);

                %% Calculate RF(Xt)                     
                [RF_X,valid_idx] = testRFmodel_FieldSample(directory,response_var_Inyear,response_var,0);
                
                %% Calculate RF(X-act, Xac2000)  
                [RF_X_minus_ac,~] = testRFmodel_FieldSample(directory,response_var_Inyear,response_var,1);
...
function [Y_pred,valid_idx] = testRFmodel_FieldSample(directory,response_var_Inyear,response_var,use_2000_climateTAC)
    ...
    if use_2000_climateTAC
        filename = fullfile(directory,'Input',response_var_Inyear,'Sample_multipleInPlot_HPC_input_use_2000_climateTAC_updated.csv');
    else
        filename = fullfile(directory,'Input',response_var_Inyear,'Sample_multipleInPlot_HPC_input_updated.csv'); % Output CSV file
    end
    T = readtable(filename);
    % TODO:
