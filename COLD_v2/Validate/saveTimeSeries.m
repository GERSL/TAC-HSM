...
% Tis function save time series of sample points for quicker loading.
%   The output of this script script should be saved as line_t_rxxxxxcxxxxx.mat file 
%   under the 'sampleTS' foler, which can then be loaded in the 'COLDsamples.m'.
...
addParameter(p,'ARDTiles',['18TYM']); % default tile is T18TYM.
addParameter(p,'sensor',['HLS']); % default sensor is HLS, can be L30, S30, or Sentinel-2(S2).
...
% ARDTiles = {'18TYL','18TXL','18TYM','18TXM'};
ARDTiles = {'18TXM'};
sensor = {'S2'};
% sensor = {'HLS10'}; % 10 m HLS
for iARD = 1: length(ARDTiles)
    hv_name = ARDTiles{iARD};

    %% set input path here
    folderpath_cold = fullfile('/shared/cn450/Sentinel-2/COLDResults/',hv_name);
%     folderpath_cold = fullfile('/shared/cn452/Kexin/COLDResults/',hv_name);
%     folderpath_cold = fullfile('/shared/cn450/Kexin/COLDTIFResults/',hv_name,'TimeSeriesPerPixel');
...
%     folderpath_samplets = fullfile(folderpath_cold,'SurveySampleTS');
...
%     %% load samples here
%     % validataion samples
%     samplecsv = dir(fullfile('/shared/cn449/Kexin/Samples/',hv_name,'*samples*S2*.csv'));
%     % field survey samples
% %     samplecsv = dir(fullfile('/shared/cn452/Kexin/Survey/','*samples*HLS.csv'));
...
%         pt_col = cols(i);
        ...
        [sdate, line_t] = readStackLineDataSample(folderpath_stackrows, metadata.ncols, metadata.nbands, pt_row, pt_col, [],sensor);
