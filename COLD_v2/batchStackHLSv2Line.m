function batchStackHLSv2Line(varargin)
...
% COPYRIGHT @ GERSLab
    ...
    addParameter(p,'nsubrow',1);       % default is saving output for each row
    addParameter(p,'sensor','HLS');   % default is only stack L30 images
...
%         folderpath_ard =  fullfile(path_ard, tile_name);
        ...
        if resolution==30
            ...
            folderpath_out = fullfile(path_working, tile_name, sprintf('StackData%s60m',num2str(nsubrow)));
        end
        ...
        stackHLSv2Line(folderpath_ard, folderpath_out, ...
             'check', true,'nsubrow', nsubrow, 'msg',true, 'task', task ,'ntasks', ntasks,...
