function ShowLandDistMap(varargin)
...
addpath('/home/kes20012/MatlabCOLD.Example/COLD_v13_04');
...
addParameter(p,'StartYear',start_year);
addParameter(p,'EndYear',end_year);
addParameter(p,'TileDir','');

parse(p,varargin{:});
start_year=p.Results.StartYear;
end_year=p.Results.EndYear;
tiledir = p.Results.TileDir;
% tiledir = '/scratch/kes20012/S2_stack/T18TYL/';


% cd('/gpfs/sharedfs1/zhulab/Kexin/S2_CT_stack/T18TXL/');
fprintf('Start year is %d \r',start_year);
fprintf('End year is %d \r',end_year);
fprintf('TileDir is %s \r',tiledir);
cd(tiledir);
...
% read in ENVI hdr
% [~,info,R] = rs_imread('/gpfs/sharedfs1/zhulab/Kexin/ROWmask_CT/ROWmask_T18TXL.tif');
...
l_dir = pwd;
% skx: update map_info and coordinate_system
infodir = '/gpfs/sharedfs1/zhulab/Kexin/S2_Tile_Extent/';
tile = tiledir(end-6:end-1);
info1 = read_envihdr(fullfile(infodir,strcat(tile,'.HDR')));
info.map_info = info1.map_info;
info.coordinate_system_string = info1.coordinate_system_string;
