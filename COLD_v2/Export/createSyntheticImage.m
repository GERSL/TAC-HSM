function createSyntheticImage(folderpath_cold, yy, mm, dd, varargin)
...
%             s_date = yy*1000 + doy;
    ...
    %% Image folder
    folderpath_syn = fullfile(folderpath_cold, 'SyntheticImage');

    
    %% get metadata
    load(fullfile(folderpath_cold, 'metadata.mat'));
    
    filename_syn = sprintf('SS_%s_%d', metadata.tile, s_date);  % S: Synthetic % S: Sentinel-2
    ...
    if isfolder(fullfile(folderpath_syn, filename_syn))
        files_have = dir(fullfile(folderpath_syn, filename_syn, 'SS*.tif'));
        if length(files_have) == metadata.nbands
            if msg
                fprintf('Already exist synthetic image %s\r\n', filename_syn);
            end
            return;
