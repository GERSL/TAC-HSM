...
%     folderpath_cold = '/shared/cn450/Sentinel-2/COLDResults/18TYM/';
    folderpath_cold = '/shared/cn450/Kexin/COLDTIFResults/10SFG/';
end
if ~exist('years', 'var')
    years = [2015:2021];
end
...
if doTIF
    ...
    folderpath_chgmap = fullfile(folderpath_cold, 'CCDCMap_t=1day_robustfit');
else
    folderpath_tsf = fullfile(folderpath_cold, 'TSFitLine_noTIF');
    folderpath_chgmap = fullfile(folderpath_cold, 'CCDCMap_noTIF');
end
if ~isfolder(folderpath_chgmap)
    mkdir(folderpath_chgmap);
end

% dirExtent10 = dir(fullfile('/scratch/zhz18039/kes20012/COLDS2Results/10SFG/MaskImage/','*.tif'));   % 10 m
...
for line = 1: num_line
    ...
    for i = 1:l_pos - 1 % -1: segment of time series will not have change record!
        ...
        if change_prob(i) == 1
            [break_type,break_year,break_doy] = labelDisturbanceType(coefs(:,:,i),t_break(i),t_min,mag(:,i),coefs(:,:,i+1),sensor);
            if ctype
...
for i_yr = 1: length(years)
    yr = years(i_yr);
%     imgGridobjLike10.Z = uint16(LandDistMap(:,:,i_yr));
...
%     GRIDobj2geotiff(imgGridobjLike10, fullfile(folderpath_chgmap, filenameout));
end
