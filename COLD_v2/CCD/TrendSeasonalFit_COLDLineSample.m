function [rec_cg, clrx, clry] = TrendSeasonalFit_COLDLineSample(sdate, line_t, path_t, path_r, ncols, nrows, proc_cols, T_cg,Tmax_cg,conse,num_c,nbands,B_detect)
...
%     line_m = line_t(:,nbands*i_ids);
    line_m = line_t(:,nbands);  % skx 07/05/2021
...
%     line_t(:,nbands*(i_ids-1)+7) = 10*(line_t(:,nbands*(i_ids-1)+7) - 2732);
    ...
    idrange = line_t(:,1)>0&line_t(:,1)<10000&...
        ...
        line_t(:,7)>0&line_t(:,7)<10000&...
        line_t(:,8)>0&line_t(:,8)<10000&...
        line_t(:,9)>0&line_t(:,9)<10000&...
        line_t(:,10)>0&line_t(:,10)<10000;
...
%                 clry = line_t(idgood,(nbands*(i_ids-1)+1):(nbands*(i_ids-1)+nbands-1));
                clry = line_t(idgood,1:(nbands-1));  % skx 07/25
                clry = double(clry);
                % find repeated ids
