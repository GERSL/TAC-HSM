function [rec_cg, clrx, clry] = TrendSeasonalFit_COLDLinePS(sdate, line_t, path_t, path_r, ncols, nrows, proc_cols, T_cg,Tmax_cg,conse,num_c,nbands,B_detect)
% Continuous Change Detection (CCD) designed for Continuous mOnitoring of
% Land Distubance (COLD) for the CONUS.
% COLD Version 2
...
%
%% Revisions: $ Date: 1/1/2021 $ Copyright: GERS Lab, UCONN
...
% ks 20240118: change to NIR since PS does not have SWIR bands
num_B2 = 4;
% num_B2 = 5;
...
rec_cg = struct('t_start',[],'t_end',[],'t_break',[],'coefs',[],'rmse',[],...
    'pos',[],'change_prob',[],'num_obs',[],'category',[],'magnitude',[],'durchange',[]);
% ks: add clrx
% rec_cg = struct('t_start',[],'t_end',[],'t_break',[],'coefs',[],'rmse',[],...
%     'pos',[],'clrx',[],'change_prob',[],'num_obs',[],'category',[],'magnitude',[],'durchange',[],'clrx',[]);
...
%     line_t(:,nbands*(i_ids-1)+7) = 10*(line_t(:,nbands*(i_ids-1)+7) - 2732);
    ...
    % skx: S2 does not have the thermal band
    ...
    % idclr = line_m < 2;
    idclr = line_m ==1; % ks 20240910 update the clear pixel value
    ...
    if sum(idclr) < n_times*max_num_c
        ...
        if sn_pct > t_sn
            ...
            if n_sn < n_times*min_num_c % not enough snow pixels
                ...
                tmp_y = zeros(length(clrx),nbands-1);
                % get the mean values
                for i = 1:nbands-1
                ...
                % record durchange
                rec_cg(num_fc).durchange = zeros(2,nbands-1);
            end
        ...
        % get the mean values
        % for i = 1:nbands-1
        % ks 20240118: modify i = 1:4 since PS only has four bands
        for i = 1:4
            tmp_y(:,i) = clry(uniq_id,i);
        end
        clry = tmp_y;
