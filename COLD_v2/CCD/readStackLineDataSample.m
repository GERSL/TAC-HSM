function [sdate, line_t, path_t]= readStackLineDataSample(folderpath_row, ncols, nbands, rows, cols, path_r,sensor)
...
%     singlepath = [];
    ...
    imf = dir(fullfile(folderpath_row,'T18*'));
    % FOR S2
...
%   % FOR HLS
    ...
    yr = str2num(imf(:, 7:10)); %#ok<*ST2NM>
    doy = str2num(imf(:, 11:13));
    sdate = datenum(yr, 1, 0) + doy;
    if ~isempty(path_r)
        path_t = str2num(imf(:,23:25)); % record the path of Landsat swath
    end
    
    %% Read Sentinel-2 time series data
    ncol = length(cols);
    line_t = zeros(num_t,nbands*ncol, length(rows), 'uint16'); %Ys
    % at each S2 image
    for i = 1:num_t
...
%                 line_t(i,:,:) = fread(fid_t, [nbands*ncols, length(rows)],'int16','ieee-le'); % get Ys
        ...
        for ir = 1: length(irows)
            % move forward the irows(ir) and icol(ic) that is the real index of the row
            % dataset
            icols = cols;
            for ic = 1: length(icols)
                fseek(fid_t,2*(irows(ir)-1)*ncols*nbands+2*(icols-1)*nbands,'bof'); % 2 means uint16
                % ir is the index of line_t
%                 line_t(i,:,ir) = fread(fid_t,nbands*ncols,'int16','ieee-le'); % get Ys
                line_t(i,:,ir) = fread(fid_t,nbands,'int16','ieee-le'); % get Ys
            end
        end
    end
