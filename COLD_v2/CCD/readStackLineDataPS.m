function [sdate, line_t, sensorgroup, path_t]= readStackLineDataPS(folderpath_row, ncols, nbands, rows, path_r)
...
%     singlepath = [];
    ...
    imf = dir(fullfile(folderpath_row,'20*'));
    yeardoy = regexpi({imf.name},'(\d{7})','match'); 
    imf = regexpi({imf.name}, '20(\w*)_(\w*)', 'match'); %Lxxx_Path
    
    % if strcmp(sensorname,'S30')
    %     imf = regexpi({imf.name}, '\w*(S30)', 'match');
    % elseif strcmp(sensorname,'L30')
    %     imf = regexpi({imf.name}, '\w*(L30)', 'match');
    % elseif strcmp(sensorname,'HLS')
    %     imf = regexpi({imf.name}, '\w*(L30|S30)', 'match');
    % end
    yeardoy = [yeardoy{:}];
    yeardoy = vertcat(yeardoy{:});
    % yeardoy = [string(yeardoy)];
    imf = [string(imf)];
    ...
    [~, sort_order] = sort(str2num(yeardoy));
    imf = imf(sort_order);
    % number of files
    num_t = length(imf);
    ...
    yr = str2num(yeardoy(:, 1:4));
    doy = str2num(yeardoy(:, 5:7));
    ...
    for i = 1:num_t
        ...
        for ir = 1: length(irows)
            ...
            % drop the confidence band
            tmp = fread(fid_t,nbands*ncols,'int16','ieee-le'); % get Ys
            try
                tmp2 = reshape(tmp(:),[nbands,ncols]);
            catch
                continue;   % if the file is empty
            end
            % tmp2(6,:) = [];
            tmp3 = reshape(tmp2,[nbands*ncols,1]);
            clear tmp2
