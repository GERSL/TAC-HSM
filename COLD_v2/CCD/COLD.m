function COLD(folderpath_cold, varargin)
...
B_detect = [2,3,8,9,10];  % w/o red edge bands
% B_detect = [2:9];    % w/ red edge bands
...
ids = unique(ceil(rows/10));   % Sentinel-2
...
%         %% based on single path Landsat data, ... we don't need this
%         if singlepath
...
%                 [sdate, line_t, path_t] = readStackLineData(folderpath_stackrows, metadata.ncols, metadata.nbands, rows(ir));
%             end
%             % ccd processing
