function [sdate, line_t, sensorgroup, path_t]= readStackLineDataHLS(folderpath_row, ncols, nbands, rows, path_r,sensorname)
...
%     singlepath = [];
    ...
    imf = dir(fullfile(folderpath_row,'T*'));
...
%     imf = regexpi({imf.name}, 'L(\w*)_(\w*)', 'match'); %Lxxx_Path
    ...
    %% kexin(20220624): add outoupt 'sensorgroup' to seperate L30 and S30
    sensorgroup = string(imf(:,end-2:end));
    sensorgroup = matches(sensorgroup,'S30');  %covert to logical, 0:L30, 1:S30
