function stackHLSResampleLine(folderpath_ard, folderpath_stack, varargin)
...
%     singlepath = true;
    %% constant parameter    
    nbands = 7; % 6 spectral bands + Fmask QA band
    
    %% Filter for Sentinel-2 folders
    % get num of total folders start with "HLS"
    imfs = dir(fullfile(folderpath_ard,'HLS.L30*'));
...
%     imfs = regexpi({imfs.name}, 'T18(TXL|TXM|TYL|TYM)(\w*)', 'match');
    imfs = regexpi({imfs.name}, 'HLS.L30.T(14SPJ).(\w*).v1.4.hdf', 'match');

    
    imfs = [imfs{:}];
    if isempty(imfs)
        warning('No HLS Resample Data at %s!', folderpath_ard);
        return;
    end
...
%     imfs(:, end-2:end) = []; 
    ...
    yyyymmdd = str2num(imfs(:, 16:22)); % should change for different sets
...
%                     if rowdata_exist(iexist).bytes == metadata.nsubrows*metadata.nrows*16 % bit size at uint 16
        ...
        filepath_sr = dir(fullfile(folderpath_ard, imf, 'HLS.L30*.tif'));
        try
            if isfile(fullfile(filepath_sr(1).folder,filepath_sr(1).name))
                ...
                if task == 1 && i == 1
                    % metaset generator
                    dirfmask = dir(fullfile(folderpath_ard, imf, 'HLS.L30*QA_10m.tif'));
%                     dirfmask = dir(fullfile(folderpath_ard, imf, 'T*B08_10m.tif'));
        ...
        %             cfmask0 = GRIDobj(fullfile(folderpath_, [imf, 'T*mask4*.tif']));
                ...
                tif_B2 = dir(fullfile(folderpath_ard,imf,'HLS*Blue*.tif'));
                tif_B3 = dir(fullfile(folderpath_ard,imf,'HLS*Green*.tif'));
                tif_B4 = dir(fullfile(folderpath_ard,imf,'HLS*Red*.tif'));
                tif_B5 = dir(fullfile(folderpath_ard,imf,'HLS*NIRN*.tif'));
                tif_B6 = dir(fullfile(folderpath_ard,imf,'HLS*SWIR1*.tif'));
                tif_B7 = dir(fullfile(folderpath_ard,imf,'HLS*SWIR2*.tif'));
...
%                     linedata(:,:,6) = b6(irow: irow_end, :); % red edge 3            
                    ...
                    linedata(:,:,6) = b6(irow: irow_end, :); % swir2
