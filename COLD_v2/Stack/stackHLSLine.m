function stackHLSLine(folderpath_ard, folderpath_stack, varargin)
...
%     folderpath_stack = '/shared/cn450/DataHLSv1.4/18TYM_Stack/';
%     folderpath_stack = '/scratch/kes20012/S2_stack_fusion_single/T18TYL/';
    ...
    addParameter(p,'sensor', 'HLS');  % default is HLS. This function also support stacking L30 and S30.
    ...
    msg = 'True';
%     tileNameHLS = 'T18TYM';
    tileNameHLS = ['T' folderpath_ard(end-4:end)];
    %% constant parameter    
    nbands = 7; % 6 spectral bands (R,G,NNIR,SWIR2,SWIR2) + Fmask QA band
 
    %% Filter for HLS folders
    if strcmp(tileNameHLS,'T18TXM')
        folderpath_ard = '/gpfs/sharedfs1/zhulab/Su/HLS14/18TXM';
    end
...
%     for k = 5
    for k = 1:length(imgDir1)
        ...
        for j = 1:length(imgDir2)
            ...
            imgsHLS = dir(fullfile(imgPath3,'HLS.*.hdf'));
            hdfFolder = [hdfFolder;vertcat(imgsHLS.folder)];
%             % e.g., name HLS.L30.T18TYM.2018001.v1.4.hdf
            if strcmp(sensor,'HLS')
                imgsHLS = regexpi({imgsHLS.name}, ['HLS.(L30|S30).', tileNameHLS,'.(\w*).(\w*).(\w*)','.hdf'], 'match');
            elseif strcmp(sensor,'L30')
                imgsHLS = regexpi({imgsHLS.name}, ['HLS.(L30).', tileNameHLS,'.(\w*).(\w*).(\w*)','.hdf'], 'match');
            elseif strcmp(sensor,'S30')
                imgsHLS = regexpi({imgsHLS.name}, ['HLS.(S30).', tileNameHLS,'.(\w*).(\w*).(\w*)','.hdf'], 'match');
            end          
            imgsHLS = [imgsHLS{:}];
            if isempty(imgsHLS)
%                 return;
    ...
    if msg
        fprintf('A total of %04d HLS images at %s\r\n',numHLSImage, folderpath_ard);
    end
    ...
    for i = start_i:end_i
        ...
        %% first check exist or not by DIR the last row folder
        if checkexist
...
%                     end
        ...
        if task == 1 && i == 1
            % metaset generator
            if resolution==30
                geotiff = GRIDobj(fullfile('/shared/cn450/DataHLSv1.4/GeoTIFF/',[tileNameHLS(2:end) '.tif']));
            else
                geotiff = GRIDobj(fullfile('/shared/cn450/DataHLSv1.4/GeoTIFF/',[tileNameHLS(2:end) '_60m.tif']));
            end

%             geotiff = GRIDobj(fullfile(imgPath1(1:end-5),'GeoTIFF',[tileNameHLS(2:end) '.tif']));
            metadata = [];
        ...
        try
            path_img = fullfile(hdfFolder(i,:),imgName);
            pixEdge = [nrows, ncols];
            pixLoc = [1 1];
            y_ref = zeros(nrows, ncols, nbands, 'int16'); %Ys
            if strcmp(imgName(5:7), 'S30')
                img = readS30(path_img, pixLoc, pixEdge);
                y_ref(:,:,1) = img.Blue;
                y_ref(:,:,2) = img.Green; 
                y_ref(:,:,3) = img.Red; 
                y_ref(:,:,4) = img.NIRN; 
            ...
            elseif strcmp(imgName(5:7), 'L30')
                img = readL30(path_img,pixLoc, pixEdge*2);
                if resolution==30
                    y_ref(:,:,1) = img.Blue;
                    y_ref(:,:,2) = img.Green; 
                    y_ref(:,:,3) = img.Red; 
                    ...
                    y_ref(:,:,1) = imresize(imgaussfilt(img.Blue,2), pixEdge,'bicubic');
                    y_ref(:,:,2) = imresize(imgaussfilt(img.Green,2), pixEdge,'bicubic');
                    y_ref(:,:,3) = imresize(imgaussfilt(img.Red,2), pixEdge,'bicubic');
                    y_ref(:,:,4) = imresize(imgaussfilt(img.NIRN,2), pixEdge,'bicubic');
                    y_ref(:,:,5) = imresize(imgaussfilt(img.SWIR1,2), pixEdge,'bicubic');
                    y_ref(:,:,6) = imresize(imgaussfilt(img.SWIR2,2), pixEdge,'bicubic');
                end
            end
            % convert pixel QA to fmask values
            cfmask = zeros(pixEdge*2);  % 0: clear land pixel
            cfmask0 = img.QA;
            ...
            if resolution==30
                ...
                y_ref(:,:,7) = imresize(cfmask,[nrows,ncols],'nearest');  
            end
...
%             for irow = 1: nrowsper:10
                ...
                linedata(:,:,1) = y_ref(irow: irow_end,:,1); % blue
                linedata(:,:,2) = y_ref(irow: irow_end,:,2); % green
                linedata(:,:,3) = y_ref(irow: irow_end,:,3); % red
                linedata(:,:,4) = y_ref(irow: irow_end,:,4); % nnir
                linedata(:,:,5) = y_ref(irow: irow_end,:,5); % swir1
                linedata(:,:,6) = y_ref(irow: irow_end,:,6); % swir2
                linedata(:,:,7) = y_ref(irow: irow_end,:,7); % qa
